...
export default function WeekDayHeader({ day, index, width, isToday, isTargetDay, showRevenue, t, lang, onOpenDay, colorOf, opFirsts, setOpTip }) {
  ...
  return (
    <div style={{ flex: '0 0 ' + width + 'px', minWidth: 0, borderLeft: WEEK_DAY_BORDER, background: isToday ? WEEK_TODAY_BG : 'transparent', boxShadow: isTargetDay ? 'inset 0 -2px 0 var(--ink)' : 'none', transition: 'box-shadow 100ms' }}>
      <button onClick={() => onOpenDay(day.date)} style={{ display: 'block', width: '100%', textAlign: 'center', padding: '8px 4px 5px', background: 'transparent', border: 'none', cursor: 'pointer' }}>
